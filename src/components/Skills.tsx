            ...
            <div className="group relative flex flex-wrap justify-center col-span-3 gap-2 md:gap-5 p-2 lg:p-5  md:p-14  overflow-hidden rounded-xl">
                {
                    SkillsData.map((data, i) => (
                        <Skillscard key={i} name={data.skill_name}>
                            {data.Icon}
                        </Skillscard>
                    ))
                }
...
function Skillscard({ children, name }: { children: JSX.Element, name: string }) {
    return (
        <ShineBorder className=' rounded-md'>
            <div className=' px-4 py-2 flex gap-3 rounded-md bg-white dark:bg-black text-neutral-500 text-sm m-[1px] transform transition duration-200 hover:shadow-md hover:shadow-blue-600'>
